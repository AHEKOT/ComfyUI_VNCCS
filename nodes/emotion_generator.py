...
class EmotionGeneratorDebug(EmotionGenerator):
    ...
    RETURN_TYPES = ("IMAGE", "STRING", "STRING", "STRING", "STRING", "STRING", "INT", "MASK", "STRING")
    RETURN_NAMES = ("images", "emotions_out", "face_output_paths", "sheet_output_paths", "positive_prompt",
                    "negative_prompt", "seed", "masks", "emotion_names")
    OUTPUT_IS_LIST = (True, True, True, True, False, False, False, True, True)
    ...
    def generate_all_emotions(self, character):
        ...
        # Call the original method to get the base outputs
        images, emotions_out, face_output_paths, sheet_output_paths, positive_prompt, negative_prompt, seed, masks = self.generate_emotions(character, "DEBUG_MODE", fake_input_string)
        
        # Extract safe_names from emotions_out in the same order
        # emotions_out format: "(safe_name, description), face_details"
        emotion_names = []
        for emotion_text in emotions_out:
            # Extract safe_name from the start of the string
            match = re.match(r"\(([^,]+),", emotion_text)
            if match:
                emotion_names.append(match.group(1).strip())
            else:
                emotion_names.append("")
        
        # Return all outputs including the new emotion_names list
        return images, emotions_out, face_output_paths, sheet_output_paths, positive_prompt, negative_prompt, seed, masks, emotion_names
